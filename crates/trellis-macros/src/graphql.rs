...
fn generate_field_registration(method: &MethodInfo) -> TokenStream {
    ...
    let type_ref = if ret.is_unit {
        quote! { TypeRef::named_nn(TypeRef::BOOLEAN) }
    } else if ret.is_option {
        // Option<T> -> nullable T
        quote! { TypeRef::named(TypeRef::STRING) } // simplified for now
    } else if ret.is_result {
        quote! { TypeRef::named_nn(TypeRef::STRING) } // simplified for now
    } else {
        quote! { TypeRef::named_nn(TypeRef::STRING) } // simplified for now
    };
    ...
    let method_name_literal = method_name.clone();

    quote! {
        let field = Field::new(#field_name, #type_ref, move |ctx| {
            let service = service.clone();
            let method = #method_name_literal;
            FieldFuture::new(async move {
                // For now, return a simple JSON value
                // Full implementation would extract args and call the actual method
                Ok(Some(::async_graphql::Value::String("todo".to_string())))
            })
        })
