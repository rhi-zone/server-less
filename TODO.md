...
- [x] **`#[server(config = MyConfig)]`** ✅ Generates `config_subcommand()` + `config_run_subcommand()` on the server struct; supports `config_cmd = "name"` / `config_cmd = false`. 3 tests in `server_tests.rs`.
- [x] **Config subcommand** ✅ 4 subcommands: show, schema, validate, set. Available on both `#[program]` and `#[server]`.
- [x] **Integration test for `#[derive(Config)]`** ✅ Tests in `program_tests.rs` cover defaults loading, field meta, env var mapping, and custom config_cmd names.
- [ ] **Integration test for `#[app]`** — verify name/description flow through to CLI --help and OpenAPI title.
- [x] **Nested `Config` struct support** ✅ `#[param(nested)]` on a field whose type also `#[derive(Config)]`; TOML section = field name (overridable with `file_key`); env prefix = parent prefix + field name (overridable with `env_prefix`); `ConfigFieldMeta` gains `nested` and `env_prefix` fields; two new internal `ConfigSource` variants (`TomlTable`, `MergeTomlTable`) pass pre-extracted sub-tables to child `load()`; `flatten_toml` fixed to not quote string values. 6 integration tests in `program_tests.rs`.
- [x] **Multi-file merge semantics in `Config::load`** ✅ `ConfigSource::MergeFile(path)` added: supplements rather than replaces — only fills in fields not yet set by prior sources. For the *project-overrides-global* pattern (`File(global) → File(project)`), the flat-map approach already does the right thing: absent TOML keys don't overwrite prior values. `MergeFile` serves the reverse case (global-wins, local supplements). (Requested by normalize dogfooding.)
- [x] **`#[param(nested, serde)]` — serde-passthrough for complex nested types** ✅ Deserializes TOML sub-table via `serde::Deserialize` instead of `Config::load`. Env-var sources silently skipped for serde-nested subtrees (documented tradeoff). `MergeFile` supplement semantics respected. `Option<T>` field type supported for optional sections. `toml` re-exported as `server_less_core::__toml` for use by generated code. 5 integration tests in `program_tests.rs`. (Requested by normalize dogfooding.)
- [x] **`about` → `description` rename migration hint** ✅ Special-cased in `cli.rs` and `program.rs`: if `other == "about"`, emit "unknown argument `about` — renamed to `description` in 0.4.0" with example. Removed `"about"` from valid candidates list to prevent Levenshtein false positive. Compile-fail fixture added.
...
- **Polish round 3 ready** — Two prior polish passes (POLISH.md) and a deferred-decisions session have been completed. All POLISH.md findings are resolved. Two pre-existing test failures remain (`test_async_jq_flag`, `test_jq_flag_dispatch`) — unrelated to polish work; may need investigation. The codebase is in good shape for a fresh audit pass.
