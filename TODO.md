...
- [ ] **Nested `Config` struct support** — `#[derive(Config)]` currently only handles flat structs (each field parsed via `FromStr`). Real-world configs are hierarchical: `NormalizeConfig` has `DaemonConfig`, `AnalyzeConfig`, `RulesConfig`, etc. as named nested fields mapping to TOML sections (`[daemon]`, `[analyze]`). Proposed: `#[param(flatten)]` on a field whose type also `#[derive(Config)]`, or auto-detect `Config` implementors and recurse. Without this, callers with non-trivial configs must keep `serde::Deserialize` + manual `toml::from_str` instead of adopting `Config::load`. (Requested by normalize dogfooding.)
- [ ] **Multi-file merge semantics in `Config::load`** — `ConfigSource::File(path)` currently does last-wins override at the source level (later file replaces earlier). For the global-config + project-config pattern (`~/.config/app/config.toml` + `.app/config.toml`), callers need field-level merging: `None` fields in the project file don't erase values from the global file. Proposed: a `ConfigSource::MergeFile(path)` variant, or a merge flag on `File`. (Requested by normalize dogfooding — currently works around this with a custom `Merge` trait.)
