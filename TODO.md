...
- [ ] **`#[param(nested, serde)]` — serde-passthrough for complex nested types** — `#[param(nested)]` requires the child type to also `#[derive(Config)]`, which only works for structs with flat named fields. Types that use `#[serde(flatten)] HashMap` (e.g. `AliasConfig`, `RulesConfig` with per-rule overrides, `RuleTagsConfig`) are incompatible. Proposed: `#[param(nested, serde)]` to deserialize the sub-table via `serde::Deserialize` instead of `Config::load` — gives env-var loading for the rest of the struct while deferring complex sections to serde. (Requested by normalize dogfooding — blocks full `#[derive(Config)]` adoption on `NormalizeConfig`.)
- [ ] **`about` → `description` rename migration hint** — the 0.4.0 breaking change renamed `about` to `description` in `#[cli]`. The error "unknown argument `about` — did you mean `about`?" is a Levenshtein false positive (suggester returns the word it just rejected, because `description` is too far away). Should special-case `about` → suggest `description` explicitly, e.g. "unknown argument `about` — renamed to `description` in 0.4.0".
